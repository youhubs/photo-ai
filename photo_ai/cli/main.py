...
Examples:
  ...
  photo-ai stats                             # Show processing statistics
    ...
    parser.add_argument('--good-dir', '-g', default='photo-good',
                       help='Directory for good quality photos')
    parser.add_argument('--bad-dir', '-b', default='photo-bad', 
                       help='Directory for poor quality photos')
    ...
    process_parser = subparsers.add_parser('process', 
                                          help='Process photos with full pipeline')
    process_parser.add_argument('directory', nargs='?', 
                               help='Directory containing photos to process')
    
    # Visa command
    visa_parser = subparsers.add_parser('visa',
                                       help='Create visa photo from input image')
    visa_parser.add_argument('input_image', help='Input image file')
    visa_parser.add_argument('output_image', nargs='?', 
                            help='Output visa photo file')
    visa_parser.add_argument('--debug', action='store_true',
                            help='Enable debug mode with additional outputs')
    
    # Analyze command
    analyze_parser = subparsers.add_parser('analyze',
                                          help='Analyze photo quality without processing')
...
def _print_visa_results(result):
    ...
    print(f"Dimensions: {dims['width_mm']}x{dims['height_mm']}mm ({dims['width_px']}x{dims['height_px']}px)")
