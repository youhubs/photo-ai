...
class VisaPhotoProcessor:
    ...
    def __init__(self, config: Config):
        ...
        self.photo_height_px = int(
            self._mm_to_inches(config.visa.photo_height_mm) * config.visa.dpi
        )
    ...
    def crop_to_visa_dimensions(self, image_path: str) -> Tuple[np.ndarray, Dict]:
        ...
        # Perform final crop
        cropped = resized[
            crop_top : crop_top + self.photo_height_px, crop_left : crop_left + self.photo_width_px
        ]
    ...
    def remove_background(self, image: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            # Create final mask
            final_mask = np.where((mask == cv2.GC_FGD) | (mask == cv2.GC_PR_FGD), 255, 0).astype(
                "uint8"
            )
    ...
    def process_visa_photo(
        self, input_path: str, output_path: str = None, debug: bool = False
    ) -> Dict:
        ...
        try:
            ...
            if output_path:
                ...
                pil_image.save(
                    output_path, dpi=(self.config.visa.dpi, self.config.visa.dpi), quality=95
                )
    ...
    def _create_debug_info(
        self, input_path: str, processed_image: np.ndarray, crop_info: Dict
    ) -> Dict:
