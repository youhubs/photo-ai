...
class SmartFilenameGenerator:
    ...
    def generate_batch_filenames(
        self, photo_results: list, processing_type: str = "best"
    ) -> Dict[str, str]:
        """
        ...
        for time_key, group in timestamp_groups.items():
            if len(group) == 1:
                ...
                filename = self.generate_filename(
                    photo["original_path"], processing_type, photo["quality_score"]
                )
                filename_mapping[photo["original_path"]] = filename
            else:
                # Multiple photos for same timestamp - add sequence numbers
                # Sort by quality score (highest first)
                sorted_group = sorted(group, key=lambda x: x["quality_score"] or 0, reverse=True)

                for seq_num, photo in enumerate(sorted_group, 1):
                    filename = self.generate_filename(
                        photo["original_path"], processing_type, photo["quality_score"], seq_num
                    )
                    filename_mapping[photo["original_path"]] = filename
    ...
    def create_processing_summary(self, results: list, output_dir: str) -> str:
        """
